...
public class CFUtil {
	 ...
	 */
	public String cfByScoreBaseUser(UserInformation cUser, DataModel model){
		log.info("******基于用户根据商品评分推荐开始******");
		//推荐结果
		String cfitemids = "";
		if(model==null){
			log.info("***用户-商品评分矩阵为空***");
		}else{
			try {
				// 定义相似度计算对象，余弦算法，
	 ...
	 * @return
	 */
//	public String cfByOrderitemBaseUser(UserInformation cUser,List<Orderitem> orderitemList){
