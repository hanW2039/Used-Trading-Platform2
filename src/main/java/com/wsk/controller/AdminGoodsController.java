...
//        request.setAttribute("path", "edit");
//        ShopInformation s = shopInformationService.selectByPrimaryKey(Integer.valueOf(goodsId.toString()));
...
//
    ...
    @ResponseBody
    public Result delete(@RequestBody Long[] ids, @PathVariable("sellStatus") Integer sellStatus) {
        if (ids.length < 1) {
            return ResultGenerator.genFailResult("参数异常！");
        }
        ...
        for (Long id : ids) {
            ...
            shopInformation.setId(Integer.valueOf(id.toString()));
            shopInformation.setStatus(Byte.valueOf(sellStatus.toString()));
